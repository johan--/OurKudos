class VirtualMergesController < ApplicationController
  ...
  def create
    ...
    if @merge.save        
      message = I18n.t(:merge_instructions_sent) if type == 'email'
      message = 'Your Twitter account has been merged' if type == 'twitter'
      redirect_to user_path(current_user), :notice => message
    else
      message = 'There was a problem merging you account' if type != 'twitter'
      message = I18n.t(:please_authorize_with_twitter_before_adding) if type == 'twitter'
